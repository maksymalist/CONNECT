...
export default function FinishedScreen({ match, user }) {
    return (
        <div style={{height:'100vh', width:'100%', display:'flex', justifyContent:'center', alignItems:'center', flexDirection:'column'}}>
            <h1 style={{color:'white'}}>{Translations[localStorage.getItem('connectLanguage')].finishedscreen.title}</h1>
            <h2 style={{color:'white', textAlign:'center'}}>{Translations[localStorage.getItem('connectLanguage')].finishedscreen.sub}</h2>
