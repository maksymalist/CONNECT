...
function ViewQuiz() {
    ...
    const { loading, error, data: quiz } = useQuery(GET_QUIZ, {
        variables: {
            id: code
        }
    })
    ...
    const StartButton = ({ code }) => (
        <svg style={{position:'unset'}} onClick={()=>{window.location = `/play?gamecode=${code}&classid=${classid}&mode=quiz`}} id='playButtonSvg' width="69" height="100" viewBox="0 0 69 100" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div>
            {
                ...
                <img style={{width:'100%', height:'400px'}} src={quiz.quiz.coverImg || Placeholder} alt="quiz" className="view__quiz__image" />
                    <div style={{textAlign:'left', padding:'10px'}}>
                        <Typography variant="h4" component='h4'>{quiz.quiz.name}</Typography>
                        ...
                        <div style={{display:'flex', alignItems:'flex-start', flexDirection:'column'}}>
                            <div style={{display:'flex', alignItems:'center', justifyContent:'flex-start'}}>
                                {   
                                    ...
                                    <img style={{width:'30px', height:'30px', marginRight:'10px', borderRadius:'50%'}} draggable='false' src={quiz.quiz.userProfilePic} alt="quiz-img" />
                                    :
                                    <AccountCircle style={{width:'30px', height:'30px', marginRight:'10px', borderRadius:'50%'}} color='primary'/>
                                }
                                <h3>{Translations[userLanguage].quiz.by} {quiz.quiz.userName || 'undefined'}</h3>
                            </div>
                            <div>
                                {
                                    quiz.quiz.tags == undefined ?
                                    null
                                    :
                                    <div>
                                        <br></br>
                ...
                <div className="view__quiz__content__questions">
                    ...
                    borderBottom:'1px solid #c4c4c4'}}>
                        <Typography variant="h5" component='h5'>{Translations[userLanguage].quiz.questions}({quiz.quiz.questions.length})</Typography>
                        {
                            ansIsShown ?
