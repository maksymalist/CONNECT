...
export default function HostRoom(props) {
    ...
    useEffect(() => {
        ...
        socket.on('UpdatePodium', (data)=>{
            ...
            toast.success(`${data.user} ${Translations[userLanguage].alerts.playerfinishedgame}`)

            if(podium.length == playerPodiumMax){
                toast.success(`${playerPodiumMax} ${Translations[userLanguage].alerts.maxpodiumlimitreached}`)
            }
        })
    ...
    const CheckPlanStatus = async () =>{
        const res = await axios.post('https://connect-backend-2.herokuapp.com/user', { userId: JSON.parse(localStorage.getItem('user')).profileObj.googleId })
    ...
    const addPoints = async (points, userId) =>{
        ...
        const res = await axios.post('https://connect-backend-2.herokuapp.com/add-points', {
            points: points,
            userId: userId,
    ...
    const createRecentGame = async () => {
        ...
        if(props.classid !== null){
            await axios.post('https://connect-backend-2.herokuapp.com/create-recent-game', recentGame)
            ...
            Podium.map( async (player) => {
                ...
                if(player.position == 1){
                    ...
                    const res = await axios.post('https://connect-backend-2.herokuapp.com/member', { userId: userId, classId: props.classid })
                    console.log(res)
                    if(res.data){
                        addPoints(100, userId)
                        console.log('there was res.data')
                    }
                }
            })
            Podium.map( async (player) => {
